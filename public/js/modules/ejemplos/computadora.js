...
setTimeout(() => {
    ...
    // Cargar la lista de computadora al iniciar
    listar();
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            const data = { nombre, marca, precio };

            try {
                // Determinar si es actualización o creación
                if (id) {
                    // PUT - Actualizar cliente existente
                    await fetchWithAuth(`/computadora/${id}`, 'PUT', data);
                    alert('Cliente actualizado exitosamente');
                } else {
                    // POST - Crear nuevo cliente
                    await fetchWithAuth('/computadora', 'POST', data);  // ← CORREGIDO: sin ${id}
    ...
    async function listar() {
        ...
        try {
            ...
            const data = await fetchWithAuth('/computadora');

            // Limpiar contenido anterior de la tabla
            tabla.innerHTML = '';

            // Verificar si hay computadora
            if (data.length === 0) {
                tabla.innerHTML = `
                    <tr>
                        <td colspan="6" class="text-center">No hay computadora registrados</td>
                    </tr>
                `;
                return;
            }
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning"
                                onclick="editar(${c.id}, '${c.nombre}', '${c.marca}', '${c.precio || ''}')">
                                 Editar
                            </button>
            ...
            console.error('Error al listar computadora:', error);
            alert('Error al cargar los computadora');
        }
    }

    // ========== EDITAR CLIENTE ==========
    // Función global para cargar datos en el formulario
    window.editar = (id, nombre, marca, precio, direccion) => {
    ...
    window.desactivar = async (id) => {
        ...
        try {
            ...
            await fetchWithAuth(`/computadora/${id}/desactivar`, 'PATCH');  // ← CORREGIDO
    ...
    window.desaparecer = async (id) => {
        ...
        try {
            ...
            await fetchWithAuth(`/computadora/${id}`, 'DELETE');  // ← CORREGIDO
